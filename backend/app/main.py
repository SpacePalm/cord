...
_RUNTIME_MIGRATIONS: list[str] = [
    ...
    # Сессии хешированные через bcrypt (формат "$2a$...", "$2b$...") больше
    # не валидируются — мы перешли на HMAC-SHA-256 для скорости. Чистим
    # чтобы строки не висели; юзеры с такими сессиями просто перелогинятся.
    "DELETE FROM session WHERE refresh_token_hash LIKE '$2%'",
]
